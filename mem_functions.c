...
/**
 * This function takes in a block header and the size of it
 * and returns a pointer to the next block header
 **/
void* getNextHeader(BLOCK_HEADER* bHeader, int headerSize)
{
    return (BLOCK_HEADER*)(headerSize + (unsigned long) bHeader);
}
...
void* Mem_Alloc(int size)
{
    ...
    BLOCK_HEADER *payloadPTR; // This is what we will return
    ...
    while(!isLast(curr))
    {
         ...
         * Set allocated
         **/
        if((size <= (blockSize - 8)) && (!isFree(curr))) 
        {
            setAllocated(curr);
            ...
            if(payload >= 16)
            {
                ...
                curr->size_alloc = 8 + size + padding; // Sets size_alloc of the current block header + padding + size + header size
...
{
    int diff = 1; // This will be the difference between the current block and the block we are trying to find
    int found = 0; // This will check if we have found the block
    int padding = 0;
    ...
    while(!isLast(curr))
    {
        ...
        {
            diff = 8 + (unsigned long)curr - (unsigned long)ptr;
            if(diff == 0)
            {
        ...
        curr = getNextHeader(curr, getSize(curr)); // Continue search
    }
        ...
        while(!isLast(curr))
        {
            ...
            if(!isFree(curr))
            {
                if(!isFree(next))
                {
                    ...
                    {
                        // Sets doubleNext to a pointer to the last block that is free after the current
